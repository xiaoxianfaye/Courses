...
class TestBusinessLogicImpl(unittest.TestCase):
    ...
    def test_get_all_transids(self):
        self.assertEquals(['Upper', 'Lower', 'TrimPrefixSpaces', 'Reverse'], self.impl.get_all_transids())

    def test_transform_reverse(self):
        self.assertEquals('  !dlrow ,olleH  ', self.impl.transform("  Hello, world!  ", ['Reverse']))