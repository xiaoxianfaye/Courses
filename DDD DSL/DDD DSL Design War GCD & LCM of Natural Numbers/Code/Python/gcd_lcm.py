...
def extract_spec_items(solutionType, spec):
    extractionTypePf = spec[solutionType]['EXTRACTION_TYPE_PF']
    extractionTypePower = spec[solutionType]['EXTRACTION_TYPE_POWER']
    return (extractionTypePf, extractionTypePower)
...
def extract_prime_factors(extractPfType, listOfPfs):
    if extractPfType == 'COMMON':
...
def extract_powers(extractionTypePower, pfs, listOfPfPs):
    return [(pf, extract_power(extractionTypePower, extract_pf_powers(pf, listOfPfPs))) for pf in pfs]
...
def extract_power(extractionTypePower, ps):
    if extractionTypePower == 'MIN':
        return min(ps)
    return max(ps)
...
# GCD and LCM Tests
def test_extract_spec_items():
    assert ('COMMON', 'MIN') == extract_spec_items('GCD', spec())
    assert ('ALL_NP', 'MAX') == extract_spec_items('LCM', spec())

    print 'test_extract_spec_items ok'
...
def test_extract_prime_factors():
    assert [2, 3, 5] == extract_prime_factors('COMMON', [[2, 3, 5], [2, 3, 5, 7], [2, 3, 5, 7]])
    assert [2, 3, 5, 7] == extract_prime_factors('ALL_NP', [[2, 3, 5], [2, 3, 5, 7], [2, 3, 5, 7]])
...
def test_extract_powers():
    ...
    assert [(2, 1), (3, 1), (5, 1), (7, 0)] == extract_powers('MIN', pfs, listOfPfPs)
    assert [(2, 2), (3, 3), (5, 2), (7, 1)] == extract_powers('MAX', pfs, listOfPfPs)
