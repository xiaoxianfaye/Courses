...
unified_proc(SpecKey, Ns) ->
    {ExtractPfSpecItem, ExtractPowerSpecItem} = extract_spec_items(SpecKey, spec()),
    {ListOfPfs, ListOfPfPs} = prime_factorize_list(Ns),
    Pfs = extract_prime_factors(ExtractPfSpecItem, ListOfPfs),
    PfPs = extract_powers(ExtractPowerSpecItem, Pfs, ListOfPfPs),
    product_powers(PfPs).
...
extract_powers(_SpecItem, [], _ListOfPfPs) ->
    [];
extract_powers(SpecItem, [H|T], ListOfPfPs) ->
    Ps = [begin
    ...
    ExtractedP = extract_power(SpecItem, Ps),
    [{H, ExtractedP} | extract_powers(SpecItem, T, ListOfPfPs)].
