...
extract_spec_items(SolutionType, Spec) ->
    {_, SpecItems} = lists:keyfind(SolutionType, 1, Spec),
    {_, ExtractionTypePf} = lists:keyfind(extraction_type_pf, 1, SpecItems),
    {_, ExtractionTypePower} = lists:keyfind(extraction_type_power, 1, SpecItems),
    {ExtractionTypePf, ExtractionTypePower}.
...
test_extract_powers() ->
    ...
    ListOfPfPs = [[{2, 1}, {3, 2}, {5, 1}], 
                  [{2, 2}, {3, 1}, {5, 1}, {7, 1}], 
                  [{2, 1}, {3, 3}, {5, 2}, {7, 1}]],
    [{2, 1}, {3, 1}, {5, 1}, {7, 0}] = extract_powers(min, Pfs, ListOfPfPs),
    [{2, 2}, {3, 3}, {5, 2}, {7, 1}] = extract_powers(max, Pfs, ListOfPfPs),
